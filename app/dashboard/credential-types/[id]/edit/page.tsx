...
import { ArrowLeft, Plus, Trash2, Save, Eye, RefreshCw } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function EditCredentialTypePage() {
  ...
  const [originalCredentialType, setOriginalCredentialType] = useState<CredentialType | null>(null)
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      if (found) {
        setOriginalCredentialType(found)
        setName(found.name)
        ...
        if (found.schema && found.schema.properties) {
          Object.entries(found.schema.properties).forEach(([key, prop]: [string, any]) => {
            schemaProperties.push({
              name: key,
              type: prop.type || "string",
              title: prop.title || key,
              required: found.schema.required?.includes(key) || false,
              format: prop.format,
        ...
        setProperties(
          schemaProperties.length > 0 ? schemaProperties : [{ name: "", type: "string", title: "", required: false }],
        )

        // 初期サンプルデータを生成
        if (schemaProperties.length > 0) {
          setSampleData(generateSampleData(schemaProperties))
        }
      }
    }
    setLoading(false)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!originalCredentialType) {
      alert("元のCredential Typeが見つかりません")
      return
    }
    ...
    // 更新されたCredential Type
    const updatedCredentialType = {
    ...
    if (stored) {
      ...
      const updated = credentialTypes.map((ct: CredentialType) =>
        ct.id === originalCredentialType.id ? updatedCredentialType : ct,
      )
      ...
      router.push(`/dashboard/credential-types/${originalCredentialType.id}`)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/dashboard/credential-types/${originalCredentialType.id}`}>
          <Button variant="outline" size="icon">
            <ArrowLeft className="h-4 w-4" />
          </Button>
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Credential Type編集</h1>
          <p className="text-gray-600 mt-2">「{originalCredentialType.name}」の設定を変更</p>
        </div>
      </div>
